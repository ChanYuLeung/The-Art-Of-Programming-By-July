...
可以先对这个序列从小到大排序，然后输出前面的最小的k个数。

首先进行快速，然后遍历序列中前k个元素。总的时间复杂度为`O（n*logn)+O(k)=O（n*logn）`。

### 解法二

题目没有要求最小的k个数有序，也没要求最后n-k个数有序。既然如此，没有必要对所有元素进行排序。
  
1、遍历n个数，先把最先遍历到的k个数存入大小为k的数组之中  
2、对数组使用选择排序或交换排序，找到这k个数中的最大值kmax。（插入排序或选择排序查找操作需要的时间复杂度为O（k））  
3、遍历剩余n-k个数。假设每一次遍历时元素的值为x。把x与kmax比较：如果`x < kmax`，用x替换kmax，并再次找出这k个元素最大元素kmax‘代替kmax；如果`x >= kmax`，则不更新数组。

每次遍历，更新或不更新数组的所用的时间为O（k）或O（0）。整趟下来，时间复杂度为：`n*O（k）=O（n*k）`。
	
### 解法三

当然，更好的办法是维护容量为k的最大堆。原理解法二的方法相似。  

1、即用容量为k的最大堆存储最先遍历到的k个数，并假设它们是最小的k个数。
2、遍历剩余n-k个数。假设每一次遍历时元素的值为x。把x与堆顶元素比较：如果`x<kmax`，用x替换kmax，然后更新堆；否则不更新堆。

时间复杂度为:`O（k+（n-k）*logk）=O（n*logk）`。使用堆进行查找和更新的时间复杂度均为：`O(logk)`（如使用解法二：在数组中找出最大元素，时间复杂度为：`O（k））`。
...
### 解法五
《算法导论》介绍了一个随机选取主元的选择算法RANDOMIZED-SELECT，它每次都是随机选取数列中的一个元素作为主元，在0（n）的时间内找到第k小的元素，然后遍历输出前面的k个小的元素。 如果能的话，那么总的时间复杂度为线性期望时间：`O（n+k）=O（n）`（当k比较小时）。
