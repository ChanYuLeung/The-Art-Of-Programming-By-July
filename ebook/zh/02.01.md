...
首先进行快速，然后遍历序列中前k个元素。总的时间复杂度为`O（n * log n)+O(k)=O（n * log n）`。
...
2、找到这k个元素中的最大值kmax。（需要遍历这k个数，时间复杂度为O（k））  
...
在《数据结构与算法分析--c语言描述》一书，第7章第7.7.6节中，阐述了一种在最坏情况下，时间复杂度为O（N）的快速选择算法，如下述文字：
