...
每次遍历，更新或不更新数组的所用的时间为O（k）或O（0）。整趟下来，时间复杂度：`n*O（k）=O（n*k）`。
...
### 解法五
《算法导论》介绍了一个随机选取主元的选择算法RANDOMIZED-SELECT，它每次都是随机选取数列中的一个元素作为主元，在O（n）的时间内找到第k小的元素，然后遍历输出前面的k个小的元素。平均时间复杂度：`O（n+k）=O（n）`（当k比较小时）。
...
结论：RANDOMIZED-SELECT的时间复杂度为O（N），但它在最坏情况下时间的复杂度为O（N^2）。
